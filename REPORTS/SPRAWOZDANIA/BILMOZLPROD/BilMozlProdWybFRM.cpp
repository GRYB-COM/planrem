...
void __fastcall T_BilMozlProdWybFRM::FormShow(TObject *Sender)
{
 ...
 m_RokED->Text          = m_Ini->ReadIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "ROK", CurrentYear());
 m_WydzialCB->KeyValue  = -9999;
 m_WydzialCB->KeyValue  = m_Ini->ReadIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Wydzia³", -1);
 m_WydzialCBCloseUp(m_WydzialCB);
 m_WarCB->KeyValue      = -9999;
 m_WarCB->KeyValue      = m_Ini->ReadIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Warsztat", -1);
 m_MiesOdCB->ItemIndex  = m_Ini->ReadIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Miesi¹c od", 1)-1;
 m_MiesDoCB->ItemIndex  = m_Ini->ReadIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Miesi¹c do", 12)-1;
...
void __fastcall T_BilMozlProdWybFRM::FormClose(TObject *Sender, TCloseAction &Action)
{
  m_WydzialSDB->close();
 m_WarSDB->close();
 m_Ini->WriteIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "ROK", m_RokED->Text.ToIntDef(CurrentYear()));
 m_Ini->WriteIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Wydzia³",  m_WydzialCB->KeyValue.IsNull() ? -1 : m_WydzialCB->KeyValue );
 m_Ini->WriteIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Warsztat", m_WarCB->KeyValue.IsNull() ? -1 : m_WarCB->KeyValue);
 m_Ini->WriteIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Miesi¹c od", m_MiesOdCB->ItemIndex +1);
 m_Ini->WriteIniInteger("ZESTAWIENIA - Bilans mo¿liwoœci produkcyjnych", "Miesi¹c do", m_MiesDoCB->ItemIndex +1);
