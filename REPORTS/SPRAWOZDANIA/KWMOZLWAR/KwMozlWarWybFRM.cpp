...
void __fastcall T_KwMozlWarWybFRM::FormShow(TObject *Sender)
{
 ...
 m_RokED->Text          = m_Ini->ReadIniInteger("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", "ROK", CurrentYear());
 m_WydzialCB->KeyValue  = -9999;
 m_WydzialCB->KeyValue  = m_Ini->ReadIniInteger("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", "Wydzia�", -1);
 m_WydzialCBCloseUp(m_WydzialCB);
 m_WarCB->KeyValue      = -9999;
 m_WarCB->KeyValue      = m_Ini->ReadIniInteger("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", "Warsztat", -1);
 m_SumCB->Checked       = m_Ini->ReadIniBool("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", m_SumCB->Caption, false);
 m_DiagCB->Checked       = m_Ini->ReadIniBool("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", m_DiagCB->Caption, false);
}
//---------------------------------------------------------------------------
...
void __fastcall T_KwMozlWarWybFRM::FormClose(TObject *Sender, TCloseAction &Action)
{
  m_WydzialSDB->close();
 m_WarSDB->close();
 m_Ini->WriteIniInteger("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", "ROK", m_RokED->Text.ToIntDef(CurrentYear()));
 m_Ini->WriteIniInteger("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", "Wydzia�",  m_WydzialCB->KeyValue.IsNull() ? -1 : m_WydzialCB->KeyValue );
 m_Ini->WriteIniInteger("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", "Warsztat", m_WarCB->KeyValue.IsNull() ? -1 : m_WarCB->KeyValue);
 m_Ini->WriteIniBool("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", m_SumCB->Caption,m_SumCB->Checked);
 m_Ini->WriteIniBool("ZESTAWIENIA - Mo퓄iwo쐁i remontowe", m_DiagCB->Caption,m_DiagCB->Checked );
